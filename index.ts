...
async function generate(_rootDir: string, sourceFile?: string, outJsFile?: string, outTsFile?: string, outTsDefFile?: string) {
    ...
    pbtsResult = pbtsResult.replace(/\[ 'Array' \].<(\w+)>/, '$1[]')
                           .replace(/\[ 'Array' \].<(\w+).(\w+)>/, '$1.$2[]')
    pbtsResult = pbtsResult.replace(/\[ 'object' \].<string, (\w+)>/, '({ [k: string]: $1 })')
                           .replace(/\[ 'object' \].<string, (\w+).(\w+)>/, '({ [k: string]: $1.$2 })')
